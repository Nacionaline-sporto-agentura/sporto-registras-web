...
const bonusColumns = {
  ...
  amount: { label: 'Premijos dydis, Eur', show: true },
};
...
const BonusesList = () => {
  ...
  const handleSetFilters = (filters) => {
    dispatch(filterActions.setBonusFilters(filters));
  };

  const notFoundInfo: NotFoundInfoProps = {
    text: emptyState.bonuses,
  };

  return (
    <TablePageLayout title={pageTitles.bonuses}>
      <TableButtonsRow>
        ...
        <Button onClick={() => navigate(slugs.newBonus)} disabled={loading}>
          {buttonsTitles.newBonus}
        </Button>
      </TableButtonsRow>
      <RecursiveTable
        onClick={(id) => navigate(slugs.bonus(id))}
        loading={loading}
        data={tableData}
